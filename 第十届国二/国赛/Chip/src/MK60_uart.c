...
void uart3_test_handler(void)
{
    ...
    if(UART_S1_REG(UARTN[uratn]) & UART_S1_RDRF_MASK)   //接收数据寄存器满
    {
       // SerialPortRx.Data = UartGetOneByte(UART0);  
         uart_getchar (UART3,&SerialPortRx.Data);
        if( SerialPortRx.Stack < UartRxBufferLen )
        {
          ...
          {   //double data 9个通道数据校验
            ...
            {   //校验成功，进行数据解算
              ...
              push(8,(int16)(UartData[0]*10.0));	//将数据发送回去观察
              push(7,(int16)UartData[1]);	//将数据发送回去观察
              push(6,(int16)UartData[2]);	//将数据发送回去观察
