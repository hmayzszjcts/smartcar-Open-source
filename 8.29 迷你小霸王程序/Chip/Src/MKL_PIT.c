...
/*!
 *  @brief      PITn定时中断
 *  @param      PITn        模块号（PIT0~PIT1）
 *  @param      cnt         定时中断时间(单位为bus时钟周期)
 *  @since      v5.0
 *  Sample usage:
                    pit_init(PIT0, 1000);                          //定时 1000 个bus时钟 后中断
                    set_vector_handler(PIT_VECTORn,pit_hander);   // 设置中断服务函数到中断向量表里
 ...
 *  @since      v5.0
 *  Sample usage:
                        pit_close(PIT0);                         //关闭PIT0
 */
void pit_close(PITn_e pitn)
{
    PIT_Flag_Clear(pitn);                                       //清中断标志位
    PIT_TCTRL(pitn) &= ~ PIT_TCTRL_TEN_MASK;                    //禁止PIT定时器（用于清空计数值）
}