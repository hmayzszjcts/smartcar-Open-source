...
/*!
 *  @brief      将AD转换后的结果换算为相应电压值(单位为mv)（基准电压为3.3V）
 *  @param      ADCn_Ch_e    ADC通道
 *  @param      ADC_nbit     ADC精度（ ADC_8bit,ADC_12bit, ADC_10bit, ADC_16bit ）
 *  @return     转换为相应电压后的采样值
 *  @since      v5.0
 *  Sample usage:       uint16 var = adc_voltage_get(ADC0_SE10, ADC_8bit);
 */
 uint16 adc_voltage_get (ADCn_Ch_e adcn_ch, ADC_nbit bit)
 {
   	uint16 data = 0;
	uint16 final = 0;
   	data = adc_once(adcn_ch, bit);
	final = (3300 * data) / ((1 << 8) - 1);
	return final;
 }
...
void adc_start(ADCn_Ch_e adcn_ch, ADC_nbit bit)
{
    ...
    ADC_SC3_REG(ADCN[adcn])   = (0
                                   | ADC_SC3_AVGE_MASK          //开启硬件平均
                                   | ADC_SC3_AVGS(0x11)         //平均次数。0x00(4次)0x01(8次)0x10(16次)0x11(32次)
                                );
    //写入 SC1A 启动转换
