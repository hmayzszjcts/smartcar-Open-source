...
/*******************************************************/
#elif (CLOCK_SETUP == 3)
			/* SIM->CLKDIV1: OUTDIV1=0000=1·Ö,OUTDIV2=0001=2·Ö,OUTDIV3=0011=4·Ö,OUTDIV4=0011=4·Ö, */
  SIM->CLKDIV1 = (uint32_t)0x00330000u; /* Update system prescalers ÎªÁËbus 100MHz,0x01330000u¸ÄÎª0x00330000u*/
	 /*************************** 
			* Switch to FBE Mode *
	 ***************************/
			/* OSC->CR: ERCLKEN=1,??=0,EREFSTEN=0,??=0,SC2P=0,SC4P=0,SC8P=0,SC16P=0 */
  OSC->CR = (uint8_t)0x80u;//Íâ²¿²Î¿¼Ê±ÖÓOSCERCLKÊ¹ÄÜ
			/* SIM->SOPT2: MCGCLKSEL=0 */
  SIM->SOPT2 &= (uint8_t)~(uint8_t)0x01u;//MCGÊ±ÖÓÑ¡ÔñOSCCLK   £¨ÓÅÏÈ¼¶ÏàÍ¬£¬×ÔÓÒÖÁ×ó£©
	
			/* MCG->C2: ??=0,??=0,RANGE=2,HGO=0,EREFS=ÒªµÈÓÚ1£¬²»ÄÜµÈÓÚ0,LP=0,IRCS=0 */
  MCG->C2 = (uint8_t)0x25u;  //0x20Íâ²¿Ê±ÖÓÔ´·¶Î§£º³¬¸ßÆµ8~32MHZ,µ«ÎÒÃÇÊÇ50MHZÓÐÔ´¾§Õñ
			/* MCG->C1: CLKS=2,FRDIV=5,IREFS=0,IRCLKEN=1,IREFSTEN=0 */
			/*AA ;CLKS=MCGOUTCLKÊ±ÖÓÔ´£ºÍâ²¿,FRDIV=32£¨RANGE=0£©,IREFS=PLL²Î¿¼Ê±ÖÓ£ºÍâ²¿Ê±ÖÓ,IRCLKEN=1£,MCGIRCLK¼¤»î,IREFSTEN=0ÄÚ²¿²Î¿¼Í£Ö¹Ê¹ÄÜ */
  MCG->C1 = (uint8_t)0xAAu;  //MCGOUTCLKÊ±ÖÓÔ´£ºÍâ²¿²Î¿¼Ê±ÖÓFRDIV=1024
			/* MCG->C4: DMX32=0,DRST_DRS=0 */
  MCG->C4 &= (uint8_t)~(uint8_t)0xE0u;  //DCO=20~25MHz   
			/* MCG->C5: ??=0,PLLCLKEN=0,PLLSTEN=0,PRDIV=0x0F */
  MCG->C5 = (uint8_t)0x0Fu;   //PRDIV·ÖÆµ16
			/* MCG->C6: LOLIE=0,PLLS=0,CME=0,VDIV=0 */
  MCG->C6 = (uint8_t)0x00u;
	 
  while((MCG->S & MCG_S_OSCINIT_MASK) == 0u) ///ÏëµÃµ½OSCINITÎ»=1
	  {
			/* Check that the oscillator is running */
    }
  while((MCG->S & MCG_S_IREFST_MASK) != 0u) //Ï£ÍûIREFST=0£¬FLL²Î¿¼Ê±ÖÓÎªÍâ²¿Ê±ÖÓ
		{ 
			/* Check that the source of the FLL reference clock is the external reference clock. */
    }
  while((MCG->S & 0x0Cu) != 0x08u) //Ï£ÍûCLKST=1000,²Î¿¼Ä£Ê½=Íâ²¿²Î¿¼Ê±ÖÓ
		{  
  ...
  MCG->C6 = (uint8_t)0x48u;  //4Ñ¡ÔñPLL×÷ÎªPLLSµÄÊ±ÖÓ,8VCO³Ë»ýÒò×Ó32
  while((MCG->S & MCG_S_PLLST_MASK) == 0u)//Ï£ÍûPLLST=1£¬
		{
			/* Wait until the source of the PLLS clock has switched to the PLL */
    }
  while((MCG->S & MCG_S_LOCK_MASK) == 0u) //PLLÊ±ÖÓËù´æµ½ËùÐèÆµÂÊ
		{ 
			/* Wait until locked */
    }
	/******************************
